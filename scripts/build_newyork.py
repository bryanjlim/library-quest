...
records = json.load(open("data/ny_libraries.json"))
library_urls = json.load(open("data/ny_library_urls.json")) if os.path.exists("data/ny_library_urls.json") else {}
for r in records:
...
RAIL = [
  {"name":"Amtrak Empire Service","op":"Amtrak","color":"#08306b","dash":True,
   "coords":[[40.7532,-73.9822],[40.9176,-73.8581],[41.1318,-73.8288],[41.5048,-73.9840],[41.7004,-73.9209],[42.0151,-73.9310],[42.3611,-73.7004],[42.6526,-73.7562],[42.8142,-73.9396],[43.0481,-76.1474],[43.1566,-77.6088],[42.8864,-78.8784]]},
  {"name":"Amtrak Adirondack","op":"Amtrak","color":"#08306b","dash":True,
   "coords":[[40.7532,-73.9822],[41.1318,-73.8288],[41.5048,-73.9840],[41.7004,-73.9209],[42.3611,-73.7004],[42.6526,-73.7562],[43.0831,-73.7846],[44.6995,-73.4529]]},
  {"name":"LIRR","op":"MTA","color":"#0039a6","dash":False,
   "coords":[[40.7532,-73.9822],[40.7226,-73.5923],[40.7573,-72.9393]]},
  {"name":"Metro-North","op":"MTA","color":"#0039a6","dash":False,
   "coords":[[40.7532,-73.9822],[40.9176,-73.8581],[41.0534,-73.8190],[41.1800,-73.1900]]}
]
...
<title>New York Library Quest</title>
<meta name="description" content="An interactive map of New York state's 23 public library systems. Track the Libby library cards you've collected to unlock more ebooks and audiobooks.">
...
<div id="app">
  <div id="side">
    ...
    <div class="controls">
      ...
      <select id="region"><option value="">All systems</option></select>
  ...
  <div id="map">
    <div id="loading" class="loading"><div class="spinner"></div>Loading New York map&hellip;</div>
...
<div id="about" class="modal-overlay">
  <div class="modal">
    ...
    <h2>New York Library Quest &#128218;</h2>
    <p>New York has <b>23 public library systems</b>, each maintaining its own independent Libby/OverDrive collection. Every card you collect unlocks a different catalog of ebooks and audiobooks.</p>
    <p><b>Pro tip:</b> ANY New York State resident can get free digital library cards at <b>The New York Public Library</b> (NYPL), <b>Brooklyn Public Library</b>, and <b>Queens Public Library</b> &mdash; three of the largest Libby collections in the country &mdash; regardless of which county you live in.</p>
    <p><b>Dot color</b> = system type. Consolidated (NYC systems), Federated (county-led), or Cooperative (member-library associations). Each system has its own independent Libby catalog.</p>
    <p class="muted">Each pin is one of the 23 public library <b>systems</b>. Click a dot for details. Progress saves in your browser.</p>
...
const CITIES=[
  {n:"New York",lat:40.7532,lon:-73.9822},{n:"Buffalo",lat:42.8864,lon:-78.8784},
  {n:"Rochester",lat:43.1566,lon:-77.6088},{n:"Syracuse",lat:43.0481,lon:-76.1474},
  {n:"Albany",lat:42.6526,lon:-73.7562},{n:"Yonkers",lat:40.9312,lon:-73.8987},
  {n:"Poughkeepsie",lat:41.7004,lon:-73.9209},{n:"Utica",lat:43.1009,lon:-75.2327},
  {n:"Watertown",lat:43.9748,lon:-75.9108},{n:"Plattsburgh",lat:44.6995,lon:-73.4529}
];
const TOTAL=DATA.filter(d=>d.libby).length;
const KEY="library_quest_ny_v1";
...
const CAT_ORDER=["Consolidated","Federated","Cooperative","No Libby"];
const CAT_COLOR={"Consolidated":"#e377c2","Federated":"#1f77b4","Cooperative":"#c2cbd9","No Libby":"#2b2b2b"};
const CAT_LABEL={"Consolidated":"Consolidated","Federated":"Federated","Cooperative":"Cooperative","No Libby":"No Libby"};
const CAT_KEY_LABEL={"Consolidated":"Consolidated (NYC systems)","Federated":"Federated (county-led)","Cooperative":"Cooperative (member libraries)","No Libby":"No Libby"};
const catKey=d=>!d.libby?"No Libby":d.system;
const catColor=d=>CAT_COLOR[catKey(d)]||"#c2cbd9";
const gmapUrl=d=>"https://www.google.com/maps/search/?api=1&query="+encodeURIComponent(d.name+", "+d.city+", NY");
const catalogName=s=>s;
...
d3.json(US).then(us=>{
  ...
  const ny=states.find(d=>d.id==="36");
  const counties=topojson.feature(us,us.objects.counties).features.filter(d=>String(d.id).slice(0,2)==="36");

  projection=d3.geoMercator().fitExtent([[20,20],[955,590]], ny);
  ...
  gState.append('path').datum(ny).attr('d',path).attr('fill','none').attr('stroke','#62748c').attr('stroke-width',1.1);
...
}).catch(err=>{ const ld=document.getElementById('loading'); if(ld) ld.remove(); mapDiv.insertAdjacentHTML('beforeend','<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#94a0b3;padding:30px;text-align:center">Could not load the base map (needs internet for the New York geometry).</div>'); console.error(err); });
...
function tipHTML(d){
  const kind = d.system + ' system \u2014 independent Libby catalog';
...
function noteHTML(d){
  if(!d.libby) return '';
  return '<div class="note">Independent Libby collection with its own titles and waitlists. Each system card unlocks a unique catalog.</div>';
}
...
function toggle(name){
  ...
  if(on) collected.add(name); else collected.delete(name);
...
function renderList(){
  ...
  rows.forEach(d=>{
    ...
    if(ckey!==cur){cur=ckey;const h=document.createElement('div');h.className='grp';
      ...
      h.innerHTML=`<span class="chev">${collapsed?'\u25b8':'\u25be'}</span><span class="gdot" style="background:${CAT_COLOR[ckey]||'#c2cbd9'};border:1px solid rgba(255,255,255,.25)"></span>${CAT_KEY_LABEL[ckey]||ckey}<span class="cnt">${n}</span>`;
...
(function(){
  let catrows=cats.map(c=>`<div class="li"><span class="swatch" style="background:${CAT_COLOR[c]||'#c2cbd9'};border-radius:50%;border:1px solid rgba(255,255,255,.3)"></span>${CAT_KEY_LABEL[c]||c}</div>`).join('');
  let seen={},rl='';
  ...
  legend.innerHTML=`<details class="catkey" open><summary>System type</summary>${catrows}</details>
...
try{ if(!localStorage.getItem('library_quest_ny_about_seen')){ aboutEl.classList.add('show'); localStorage.setItem('library_quest_ny_about_seen','1'); } }catch(e){}
...
open("newyork.html","w", encoding="utf-8").write(out)
print("wrote New York map, bytes:",len(out),"libby:",total_libby)
